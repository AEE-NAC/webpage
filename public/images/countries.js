...
function populateCountries(countryElementId, stateElementId) {
    ...
    var defaultCountry = countryElement.getAttribute('aria-default');
    console.log('default country',defaultCountry)
    if (!defaultCountry) {
        // Fetch country based on IP if aria-default is empty
        fetch('https://ipapi.co/json/')
            .then(response => response.json())
            .then(data => {
                defaultCountry = data.country_name;
                setDefaultCountry(countryElement, defaultCountry);
			if(stateElementId){
...
function setDefaultCountry(countryElement, defaultCountry) {
    for (var i = 0; i < countryElement.options.length; i++) {
        if (countryElement.options[i].value === defaultCountry) {
